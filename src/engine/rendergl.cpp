...
void renderbackground(const char *caption, Texture *mapshot, const char *mapname, const char *mapinfo, bool restore, bool force)
{
    ...
    loopi(restore ? 1 : 3)
    {
        ...
        {
            glColor4f(1, 1, 1, .4);
            glVertex2f(roffset, roffset);

            glColor4f(1, 1, 1, 0);
            glVertex2fv(rays[i].coords[0].v);
            glVertex2fv(rays[i].coords[1].v);
        }
        ...
        if(mapshot || mapname)
        {
            ...
            else
            {
                ...
                glPushMatrix();
                glTranslatef(x + (sz - FONTW * tsz) / 2 , y + (sz - FONTH * tsz) / 2, 0);
                glScalef(tsz, tsz, 1);
            ...
            if(mapname)
            {
                ...
                glPushMatrix();
                glTranslatef(x + (sz - tw * tsz) / 2, y - FONTH + sz * 0.9, 0);
                glScalef(tsz, tsz, 1);
        ...
        if(showversion == 2 || (!mapshot && !mapname && !caption && !mapinfo && showversion))
        {
            ...
            glPushMatrix();
            glTranslatef(w * .875, h * .95 - FONTH * sz, 0);
            glScalef(sz, sz, 1);
