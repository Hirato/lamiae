...
* You may need to manually specify the paths for mingw.
If necessary, direct codeblocks to find the compiler executables in C:\MingW64 (or where applicable)
the replace the compiler executables with their generic selves i.e if the executable is `mingw32-i386-unknown-gcc.exe` replace it with just `gcc.exe` - likewise `mingw32-i386-unknown-g++.exe` to `g++.exe`
